...
def node_to_metadata_dict(
    ...
    """Extrae y organiza metadatos limpios del PDF + texto del nodo."""

    node_dict = node.dict()
    original_metadata = node_dict.get("metadata", {})

    if flat_metadata:
        _validate_is_flat_dict(original_metadata)
    ...
    for key in ["file_name", "file_path", "file_type", "file_size"]:
        if key in original_metadata:
            clean_metadata[key] = original_metadata[key]

    # === 2. Fechas relevantes ===
    for key in ["creation_date", "last_modified_date", "last_accessed_date"]:
        if key in original_metadata:
            clean_metadata[key] = original_metadata[key]
    ...
    # === 4. Paginación ===
    for key in ["num_page", "total_pages"]:
        if key in original_metadata:
            clean_metadata[key] = original_metadata[key]

    # === 5. Contenido del chunk ===
    clean_metadata[text_field] = node.get_content()

    # === 6. Identificador del documento ===
    # clean_metadata["document_id"] = node.ref_doc_id or "None"
    clean_metadata["document_id"] = original_metadata.get("document_id", "None")
