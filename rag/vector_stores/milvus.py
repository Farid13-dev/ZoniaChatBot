...
class MilvusVectorStore(VectorStore):
  ...
  def __init__(
    ...
    **kwargs,
    ...
    self.doc_id_field = doc_id_field or "document_id"
    ...
    if self.collection_name not in self.milvusclient.list_collections():
      ...
      fields = [
        FieldSchema(name=MILVUS_ID_FIELD, dtype=DataType.VARCHAR, is_primary=True, max_length=64),
        FieldSchema(name=self.text_field, dtype=DataType.VARCHAR, max_length=65535),
        FieldSchema(name=self.embedding_field, dtype=DataType.FLOAT_VECTOR, dim=self.dim),
        FieldSchema(name="metadata", dtype=DataType.JSON),
        FieldSchema(name="document_id", dtype=DataType.VARCHAR, max_length=256),
      ]

      schema = CollectionSchema(fields=fields, description="RAG schema")
    ...
    self.collection = Collection(
      self.collection_name, using=self._ensure_orm_connection()
    )
    self._create_index_if_required()
    logger.debug(f"Successfully load collection: {self.collection_name}")

  def exists_document_id(self, document_id: str) -> bool:
    """Verifica si un document_id ya existe en Milvus."""
    try:
      result = self.milvusclient.query(
        collection_name=self.collection_name,
      ...
      logger.error(f"Error consultando Milvus: {str(e)}")
      return False

  def get_nodes_by_document_id(self, document_id: str) -> List[BaseNode]:
    """Recupera todos los nodos para un document_id específico desde Milvus."""
    try:
      results = self.milvusclient.query(
        collection_name=self.collection_name,
        filter=f"document_id == \"{document_id}\"",
      ...
      logger.error(f"❌ Error recuperando nodos para document_id={document_id}: {e}")
      return []

  def _ensure_orm_connection(self) -> str:
    """Registra una conexion ORM y devuelve su alias.

    pymilvus 3.0 reescribio el gestor de conexiones: MilvusClient._using pasa
    a ser un id interno ("cm-1611474531248") que el registro clasico de
    `connections` NO conoce, asi que
        Collection(nombre, using=self.milvusclient._using)
    fallaba con ConnectionNotExistException. En 2.5 ese alias si estaba
    registrado. Abrimos aqui la conexion ORM de forma explicita, lo que
    funciona tanto en 2.5 como en 3.x.
    """
    from pymilvus import connections

    uri = self.uri
    if not uri and self.host and self.port:
      uri = f"http://{self.host}:{self.port}"
    if not uri:
      raise Exception("Cannot connect milvus database")

    alias = f"zonia_{abs(hash(uri)) % 10 ** 8}"
    try:
  ...
  def add(self, nodes: List[BaseNode], **add_kwargs: Any) -> List[str]:
    ...
    for node in nodes:
      full_metadata = node_to_metadata_dict(node)

      entry = {
        MILVUS_ID_FIELD: node.node_id,
        self.embedding_field: node.embedding,
        self.text_field: node.get_content(),
        "metadata": full_metadata,  # El JSON completo
  ...
  def delete_iddocument(self, ref_doc_id: str, **delete_kwargs: Any) -> None:
    """
    Elimina todos los nodos en Milvus asociados a un `document_id`.
    """
    doc_ids = [ref_doc_id] if isinstance(ref_doc_id, str) else ref_doc_id
    quoted = ','.join(f'"{doc}"' for doc in doc_ids)
    filter_expr = f"{self.doc_id_field} in [{quoted}]"
    ...
    try:
      ...
      pks = [row["pk"] for row in results]
      if not pks:
        logger.info(f"🔍 No se encontraron nodos con document_id={doc_ids}")
        return

      self.milvusclient.delete(
        collection_name=self.collection_name,
        pks=pks
      )
      logger.info(f"🗑️ Documentos con document_id={doc_ids} eliminados correctamente. Total: {len(pks)}")
    except Exception as e:
      logger.error(f"❌ Error eliminando document_id={doc_ids}: {e}", exc_info=True)
      raise
  ...
  def query(self, query: VectorStoreQuery, **kwargs: Any) -> VectorStoreQueryResult:
    ...
    if len(expr) != 0:
      ...
      logger.debug(f"[Milvus] Filtro aplicado: {string_expr}")
    ...
    for hit in res[0]:
      ...
      metadata = entity.get("metadata", {})  # 👈 Extraemos los metadatos reales

      node = TextNode(
        text=text,
        metadata=metadata  # ✅ ¡Aquí los metemos en el nodo!
      )

      nodes.append(node)
      # pymilvus 3.0: Hit.__getitem__ busca la clave dentro de "entity", asi
      # que hit["id"] lanza KeyError. .id y .distance son propiedades del Hit
      # y existen tanto en 2.x como en 3.x.
