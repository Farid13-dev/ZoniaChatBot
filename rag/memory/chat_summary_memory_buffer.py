...
from rag.bridge.pydantic import (
    Field,
    PrivateAttr,
    model_validator,
    field_serializer,
...
from rag.llm.openai_llm import OpenAILLM  # Asegúrate que esté importado correctamente


from rag.memory.types import DEFAULT_CHAT_STORE_KEY, BaseMemory
from rag.storage.chat_store import SimpleChatStore
from rag.storage.chat_store.base import BaseChatStore
from rag.rag_utils import get_tokenizer

DEFAULT_TOKEN_LIMIT_RATIO = 0.75
DEFAULT_TOKEN_LIMIT = 2000
SUMMARIZE_PROMPT = "La siguiente es una conversación entre el usuario y el asistente. Escribe un resumen conciso sobre el contenido de esta conversación."
...
class ChatSummaryMemoryBuffer(BaseMemory):
    ...
    All older messages are iteratively summarized using the {OpenAILLM} provided, with
    the max number of tokens defined by the {OpenAILLM}.
    ...
    of each request to the OpenAILLM.
    """

    token_limit: int
    count_initial_tokens: bool = False
    llm: Optional[SerializeAsAny[OpenAILLM]] = None
    ...
    def from_defaults(
        ...
        **kwargs: Any,
        ...
        """
        Create a chat memory buffer from an OpenAILLM
        ...
        return cls(
            ...
            # TODO: Check if we can get the tokenizer from the OpenAILLM
    ...
    def from_dict(
        ...
        # NOTE: The OpenAILLM will have to be set manually in kwargs
        if "llm" in data:
            data.pop("llm")
    ...
    def _split_messages_summary_or_full_text(
        ...
        and which will have to be summarized by the OpenAILLM.
        """
    ...
    def _summarize_oldest_chat_history(
        ...
        """
        Use the OpenAILLM to summarize the messages that do not fit into the
        buffer.
        """
        ...
        # TODO: Maybe it is better to pass a list of history to OpenAILLM
    ...
    def _get_prompt_to_summarize(
        ...
        """Ask the OpenAILLM to summarize the chat history so far."""
