...
class SentenceSplitter(MetadataAwareTextSplitter):
    ...
    def _split(self, text: str, chunk_size: int, depth: int = 0, max_depth: int = 20) -> List[_Split]:
        """Break text into splits that are smaller than chunk size."""
        if depth >= max_depth:
            print(
                f"[⚠️] Máxima profundidad de recursión alcanzada para chunk_size={chunk_size}. Texto será devuelto tal cual.")
            return [_Split(text, is_sentence=False)]
        ...
        # Si no pudo dividirlo en más de 1 parte, lo devolvemos tal cual
        if len(text_splits_by_fns) <= 1:
            return [_Split(text, is_sentence=is_sentence)]
        ...
        for text_split_by_fns in text_splits_by_fns:
            if self._token_size(text_split_by_fns) <= chunk_size:
                ...
                recursive_text_splits = self._split(
                    text_split_by_fns, chunk_size=chunk_size, depth=depth + 1, max_depth=max_depth
                )
