    ...
    def get_response(
        ...
        **response_kwargs: Any,
        ...
        if len(text_chunks) == 1:
            ...
            # Stop recursion if all summaries are the same or recursion depth is too deep
            if len(summaries) == 1 or (len(set(summaries)) == 1):
                return summaries[0]

            # Optional: avoid infinite recursion with a max depth
            if response_kwargs.get("recursion_depth", 0) > 10:
                return summaries[0]

            return self.get_response(
                query_str=query_str,
                text_chunks=summaries,
                **{**response_kwargs, "recursion_depth": response_kwargs.get("recursion_depth", 0) + 1},
            )
